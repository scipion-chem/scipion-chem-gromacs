...
User IA Manual: MDsimulation Protocol

The MDsimulation protocol enables the execution of molecular dynamics simulations
using GROMACS within the Scipion-Chem framework. It is designed to work on
systems that have been fully parameterized and imported, allowing users to run
production simulations under defined thermodynamic conditions.

To begin, the user must provide a molecular system previously imported through
the ImportSystem protocol or generated by another GROMACS-based step. This system
must include topology and coordinate files in standard GROMACS formats, as well
as any necessary restraint, index, or parameter files that will be used during
simulation.

The protocol allows configuration of simulation length through the number of
steps or total simulation time, depending on the user's preferred time unit.
The integration time step can also be defined and should be chosen in accordance
with the use of constraints, temperature control, and the system?s stability.
Thermostat and barostat algorithms can be selected to define the thermodynamic
ensemble. The user can adjust coupling constants, target temperature, and
pressure values, enabling simulations in NVT or NPT conditions.

Initial velocities may be generated randomly from a Maxwell distribution or read
from a checkpoint or structure file. This option affects reproducibility and
continuity when running simulations in sequence. The user may also define how
frequently energies, coordinates, and trajectory frames are written to output
files. These settings determine the granularity of the data used for analysis
and influence the size of the resulting files.

The protocol supports the use of additional input files such as MDP parameter
files for fine-tuning simulation behavior. These can override default settings
to introduce advanced features such as free energy calculations, replica exchange,
or distance restraints. If custom MDP files are used, they must be consistent
with the system topology and simulation goals.

Upon completion, the protocol outputs a trajectory file containing the time
evolution of atomic coordinates, along with energy and log files that capture
the simulation process. These results can be passed to downstream protocols for
analysis, clustering, or visualization. Intermediate checkpoint files are also
generated, allowing simulation continuation or recovery in case of interruption.

In summary, the MDsimulation protocol provides a flexible and reproducible way
to perform molecular dynamics simulations in Scipion-Chem using the GROMACS
engine. It combines user-defined thermodynamic control, output customization,
and support for advanced features, enabling a wide range of applications in
molecular modeling and biophysical studies.
