...
class GromacsModifySystem(EMProtocol):
    ...
    User IA Manual: ModifyTrajectory Protocol

The ModifyTrajectory protocol is designed to process and modify molecular
dynamics trajectories generated by GROMACS. It allows users to apply a variety
of post-processing operations to trajectory files, enabling the preparation of
clean, centered, or stripped trajectories suitable for analysis, visualization,
or further simulation.

To begin, the user must provide a molecular system along with an associated
trajectory. The system serves as a reference for atom selection and coordinate
mapping. The trajectory should include the time evolution of the atomic
coordinates in a format supported by GROMACS, typically `.xtc` or `.trr`. The
protocol can operate on full systems or specific selections defined by index
groups.

Several options are available to control how the trajectory is modified.
The user may center the system within the simulation box using a defined group,
ensuring that the region of interest remains in a consistent spatial reference
across frames. This is particularly useful when molecules diffuse across periodic
boundaries. The protocol also allows removal of jumps due to periodic boundary
conditions, effectively reconstructing continuous motion of molecules throughout
the simulation.

If needed, the user can strip solvent molecules or ions from the trajectory,
focusing only on the biomolecular components. This helps reduce file size and
enhance clarity when inspecting protein or ligand dynamics. The protocol also
supports frame skipping, allowing users to reduce the number of frames by
specifying an interval, which is useful for managing large datasets during
visualization or clustering.

Additional adjustments may include fitting the trajectory to a reference frame
to remove global rotation and translation. This is commonly applied when the
goal is to analyze internal motions or to superimpose multiple simulations
consistently. All options can be combined to tailor the trajectory to the user's
analytical needs.

The output consists of a modified trajectory file in the desired format,
accompanied by updated metadata reflecting the transformations applied.
These results can be directly passed to downstream protocols within
Scipion-Chem, such as clustering, RMSD analysis, or molecular visualization.

In summary, the ModifyTrajectory protocol provides a flexible and reproducible
way to clean and preprocess GROMACS trajectories. It simplifies the preparation
of simulation data for analysis and helps users extract relevant motion and
structural information from complex molecular systems.
