...
class GetChainsWizard(Wizard):
    ...
    _targets = [(GromacsSystemPrep, ['chain_name'])]
    ...
    def getChain_Information(self, protocol):
        ...
        #             (2) for each chain list of residuinputpdbes

        structureHandler = AtomicStructHandler()

        if protocol.inputStructure.get() is not None:
            filename = os.path.abspath(protocol.inputStructure.get().getFileName())
